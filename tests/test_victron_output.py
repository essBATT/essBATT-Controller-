"""Tests for VictronOutput (Step 7)."""
...
def test_set_ccgx_value_publishes_on_change(output, mqtt_client, ccgx_data):
    result = output.set_ccgx_value("AcPowerSetPoint", 100, only_set_if_deviation_to_current_setting=True)
    assert result == 1
...
def test_set_ccgx_value_skips_when_unchanged(output, mqtt_client):
    result = output.set_ccgx_value("AcPowerSetPoint", 0, only_set_if_deviation_to_current_setting=True)
    assert result == 0
    mqtt_client.publish.assert_not_called()


def test_set_ccgx_value_force_publish_even_if_same(output, mqtt_client):
    result = output.set_ccgx_value("AcPowerSetPoint", 0, only_set_if_deviation_to_current_setting=False)
    assert result == 1
    mqtt_client.publish.assert_called_once()
...
def test_set_ccgx_value_none_name_returns_error(output, mqtt_client):
    result = output.set_ccgx_value(None, 1)
    assert result == -1
    mqtt_client.publish.assert_not_called()
...
def test_set_multis_switch_skips_when_same(output, mqtt_client):
    published = output.set_multis_switch_mode(3)  # already Mode 3
